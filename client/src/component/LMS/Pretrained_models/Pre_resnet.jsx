...
const Pre_resnet = () => {
    ...
    useEffect(() => {
        if (Pre_resnetFiles.length > 0 && !selectedFile) {
            setSelectedFile(Pre_resnetFiles[0]); // Default to first file
        }
    ...
    return (
        <div className="flex h-screen bg-gray-50 text-gray-800">
            ...
            <div className="w-64 bg-gray-800 text-white p-4 border-r border-gray-700 overflow-y-auto">
                <h2 className="text-xl font-bold mb-6 px-2">ResNet Resources</h2>
                <nav className="space-y-2">
                    {Pre_resnetFiles.map(file => (
                        <button
                            key={file.id}
