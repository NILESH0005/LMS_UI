...
const Image_Segmentation = () => {
    ...
    useEffect(() => {
        if (Image_SegmentationFiles.length > 0 && !selectedFile) {
            setSelectedFile(Image_SegmentationFiles[0]); // Default to first file
        }
    ...
    const handleFeedbackSubmit = (rating, comment) => {
        ...
        localStorage.setItem("Image_SegmentationFeedback", JSON.stringify(updatedFeedback));
